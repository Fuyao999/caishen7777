  ...
  return {
    ...
    execute: function () {
      ...
      _export("BlackMarketScene", BlackMarketScene = (_dec = ccclass('BlackMarketScene'), _dec2 = property(Button), _dec3 = property(Label), _dec4 = property(Label), _dec5 = property(ScrollView), _dec6 = property(Node), _dec7 = property(Label), _dec(_class = (_class2 = class BlackMarketScene extends Component {
        ...
        checkOpenTime() {
          var now = new Date();
          var hour = now.getHours(); // 黑市开放时间: 21:00 - 23:00

          this.isOpen = hour >= 21 && hour < 23;

          if (this.isOpen) {
            var minsLeft = (23 - hour) * 60 - now.getMinutes();
        ...
        updateItemList() {
          ...
          var _loop = function _loop(item) {
            var node = instantiate(_this.blackMarketItemPrefab);
          ...
          for (var item of this.blackMarketItems) {
            _loop(item);
          }
        }
        ...
        onBuy(item) {
          ...
          if (Math.random() < item.risk) {
            // 被天庭追查
            var penalty = Math.floor(item.price * 0.5);
...
//# sourceMappingURL=bb7c63321c48bee8b64c875d9618687c88f9de8e.js.map