    ...
    execute: function () {
      ...
      _export("FarmScene", FarmScene = (_dec = ccclass('FarmScene'), _dec2 = property(Button), _dec3 = property(Button), _dec4 = property(Button), _dec5 = property(Button), _dec6 = property(Node), _dec7 = property(Node), _dec8 = property(Node), _dec9 = property(Label), _dec(_class = (_class2 = class FarmScene extends Component {
        ...
        checkGrowth() {
          ...
          for (var slot of this.farmSlots) {
            if (slot.crop && !slot.isReady) {
              var cropConfig = this.crops[slot.crop];
              var elapsed = now - slot.plantTime;
        ...
        updateFarmUI() {
          var now = Date.now() / 1000;
          var slots = [this.cropSlot1, this.cropSlot2, this.cropSlot3];

          for (var i = 0; i < 3; i++) {
            var slot = this.farmSlots[i];
            ...
            if (!slot.crop) {
              ...
              if (slot.isReady) {
                ...
                var elapsed = now - slot.plantTime;
                var progress = Math.min(1, elapsed / cropConfig.growTime);
                var remaining = Math.ceil((cropConfig.growTime - elapsed) / 3600);
        ...
        onPlant() {
          // 找到空位
          var emptySlot = this.farmSlots.find(s => !s.crop);
          ...
          var cropType = 'wheat';
          var cropConfig = this.crops[cropType];
        ...
        onHarvest() {
          ...
          for (var slot of this.farmSlots) {
            if (slot.crop && slot.isReady) {
              ...
              for (var [key, value] of Object.entries(cropConfig.reward)) {
        ...
        onSteal() {
          ...
          var stealAmount = Math.floor(Math.random() * 50) + 50;
...
//# sourceMappingURL=20fd08977e097e22646b74b5f1641a13418cac66.js.map