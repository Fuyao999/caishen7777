      ...
      _export("EnhanceScene", EnhanceScene = (_dec = ccclass('EnhanceScene'), _dec2 = property(Button), _dec3 = property(Node), _dec4 = property(Label), _dec5 = property(Label), _dec6 = property(Label), _dec7 = property(Label), _dec8 = property(Label), _dec9 = property(Button), _dec10 = property(Button), _dec11 = property(ProgressBar), _dec12 = property(Sprite), _dec(_class = (_class2 = class EnhanceScene extends Component {
        ...
        updateUI() {
          ...
          if (this.selectedEquipment.level >= this.selectedEquipment.maxLevel) {
            ...
            var nextConfig = this.enhanceConfig[this.selectedEquipment.level + 1];
        ...
        onEnhance() {
          ...
          return _asyncToGenerator(function* () {
            if (_this.selectedEquipment.level >= _this.selectedEquipment.maxLevel) return;
            var nextConfig = _this.enhanceConfig[_this.selectedEquipment.level + 1];
            ...
            _this.enhanceBtn.interactable = false;
            yield _this.playEnhanceAnimation(); // 判定成功失败

            var random = Math.random();

            if (random <= nextConfig.rate) {
              // 成功
              _this.selectedEquipment.level++;

              _this.showSuccessEffect();

              console.log("\u5F3A\u5316\u6210\u529F\uFF01\u5F53\u524D+" + _this.selectedEquipment.level);
            } else {
              // 失败（不掉级）
              _this.showFailEffect();

              console.log('强化失败，装备未损坏');
            }

            _this.enhanceBtn.interactable = true;
        ...
        onAutoEnhance() {
          ...
          return _asyncToGenerator(function* () {
            var targetLevel = Math.min(_this2.selectedEquipment.maxLevel, _this2.selectedEquipment.level + 3);

            while (_this2.selectedEquipment.level < targetLevel) {
              yield _this2.onEnhance();
...
//# sourceMappingURL=e4425f298cc6bb503ace6d30cc31d4641d06455e.js.map